...
void main() {
	...
	// Sky rendering
		...
		// Sun disk
		float sunDisk = smoothstep(0.985, 0.987, dot(normalize(viewPos), (gbufferModelView * vec4(sunDirView, 0.0)).xyz));
		vec3 sunDiskColor = mix(10*vec3(5.0, 3.0, 20.0), vec3(0.0), smoothstep(0.985, 0.9865, dot(normalize(viewPos), (gbufferModelView * vec4(sunDirView, 0.0)).xyz)));
		...
		// Apply clouds
		#ifdef cloudsEnable
			applyEndCloudColor(eyeDir, vec3(0.0, eyeAltitude, 0.0), opaqueColor.rgb, -skyDirect);
		#endif
		...
		if(isEyeInWater == 0) {
			...
			if(waterDepth == 0.0) {
				#ifdef VolFog
					volumetricFog(opaqueColor, vec3(0.0), normalize(viewPos) * 30.0, texcoord, skyDirect * (1.0 - sunDisk));
				#else
					fog(opaqueColor, vec3(0.0), normalize(viewPos) * 30.0, skyDirect * (1.0 - sunDisk));
				#endif
			}
			// if there is water in the current pixel, render both water and atmospheric fog
			else {
				#ifdef VolWater
					waterVolumetricFog(opaqueColor, waterViewPos, normalize(viewPos) * far, texcoord, skyDirect * (1.0 - sunDisk), lightDir);
				#else
					waterFog(opaqueColor, waterViewPos, normalize(viewPos) * far, skyDirect * (1.0 - sunDisk));
				#endif

				#ifdef VolFog
					volumetricFog(opaqueColor, vec3(0.0), waterViewPos, texcoord, skyDirect * (1.0 - sunDisk));
				#else
					fog(opaqueColor, vec3(0.0), waterViewPos, skyDirect * (1.0 - sunDisk));
				#endif
			}
		}
		// fog when player is underwater
			...
			if(waterDepth != 0.0) {
				...
				#ifdef VolFog
					volumetricFog(opaqueColor, waterViewPos, normalize(viewPos) * 30.0, texcoord, skyDirect * (1.0 - sunDisk));
				#else
					fog(opaqueColor, waterViewPos, normalize(viewPos) * 30.0, skyDirect * (1.0 - sunDisk));
				#endif

				// render water fog
				#ifdef VolWater
					waterVolumetricFog(opaqueColor, vec3(0.0), waterViewPos, texcoord, skyDirect * (1.0 - sunDisk), lightDir);
				#else
					waterFog(opaqueColor, vec3(0.0), waterViewPos, skyDirect * (1.0 - sunDisk));
				#endif
			}
			else {
				#ifdef VolWater
					waterVolumetricFog(opaqueColor, vec3(0.0), normalize(viewPos) * far, texcoord, skyDirect * (1.0 - sunDisk), lightDir);
				#else
					waterFog(opaqueColor, vec3(0.0), normalize(viewPos) * far, skyDirect * (1.0 - sunDisk));
				#endif
			}
		}
	}
